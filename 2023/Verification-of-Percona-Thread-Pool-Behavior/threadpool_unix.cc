...
struct alignas(128) thread_group_t {
  ...
  int pollfd;
  int thread_count;
  std::atomic_int active_thread_count;
...
static connection_t *queue_get(thread_group_t *thread_group) noexcept {
  ...
  connection_t *c;
  bool toomany = too_many_busy_threads(*thread_group);
  std::printf("%ld %ld queue_get #0 (active: %d, waiting: %d, stalled: %i, toomany: %i)\n", sysmillis(), std::this_thread::get_id(), thread_group->active_thread_count.load(), thread_group->waiting_thread_count.load(), thread_group->stalled, toomany);

  if ((c = thread_group->high_prio_queue.front())) {
    std::printf("%ld %ld queue_get #1 (active: %d, waiting: %d, stalled: %i)\n", sysmillis(), std::this_thread::get_id(), thread_group->active_thread_count.load(), thread_group->waiting_thread_count.load(), thread_group->stalled);
    ...
    */
    ...
    std::printf("%ld %ld queue_get #2 (active: %d, waiting: %d, stalled: %i)\n", sysmillis(), std::this_thread::get_id(), thread_group->active_thread_count.load(), thread_group->waiting_thread_count.load(), thread_group->stalled);
...
static void check_stall(thread_group_t *thread_group) {
  ...
  */
  if (!thread_group->listener && !thread_group->io_event_count) {
    std::printf("%ld %ld check_stall #1> wake_or_create(active: %d, waiting: %d, stalled: %i)\n", sysmillis(), std::this_thread::get_id(), thread_group->active_thread_count.load(), thread_group->waiting_thread_count.load(), thread_group->stalled);
  ...
  */
  if (!thread_group->queue_event_count && !queues_are_empty(*thread_group)) {
    ...
    std::printf("%ld %ld check_stall #2> wake_or_create(active: %d, waiting: %d, stalled: %i)\n", sysmillis(), std::this_thread::get_id(), thread_group->active_thread_count.load(), thread_group->waiting_thread_count.load(), thread_group->stalled);
    wake_or_create_thread(thread_group);
  }
...
static connection_t *listener(thread_group_t *thread_group) {
  DBUG_ENTER("listener");
  std::printf("%ld %ld listener #0(active: %d, waiting: %d, stalled: %i)\n", sysmillis(), std::this_thread::get_id(), thread_group->active_thread_count.load(), thread_group->waiting_thread_count.load(), thread_group->stalled);
  ...
  for (;;) {
    ...
    if (cnt <= 0) {
      std::printf("%ld %ld listener poll no event(active: %d, waiting: %d, stalled: %i)\n", sysmillis(), std::this_thread::get_id(), thread_group->active_thread_count.load(), thread_group->waiting_thread_count.load(), thread_group->stalled);
    ...
    */
    ...
    std::printf("%ld %ld listener (active: %d, waiting: %d, stalled: %i, picks: %i)\n", sysmillis(), std::this_thread::get_id(), thread_group->active_thread_count.load(), thread_group->waiting_thread_count.load(), thread_group->stalled, listener_picks_event);
    ...
    if (thread_group->active_thread_count == 0) {
      ...
      std::printf("%ld %ld listener wake thread(active: %d, waiting: %d, stalled: %i)\n", sysmillis(), std::this_thread::get_id(), thread_group->active_thread_count.load(), thread_group->waiting_thread_count.load(), thread_group->stalled);
      if (wake_thread(thread_group)) {
        std::printf("%ld %ld listener wake thread failed(active: %d, waiting: %d, stalled: %i)\n", sysmillis(), std::this_thread::get_id(), thread_group->active_thread_count.load(), thread_group->waiting_thread_count.load(), thread_group->stalled);
        /*
        ...
        */
        if (thread_group->thread_count == 1) {
          /*
            ...
            pointless delay.
          */
          std::printf("%ld %ld listener create worker(active: %d, waiting: %d, stalled: %i)\n", sysmillis(), std::this_thread::get_id(), thread_group->active_thread_count.load(), thread_group->waiting_thread_count.load(), thread_group->stalled);
...
static int create_worker(thread_group_t *thread_group,
  ...
  std::printf("%ld %ld create worker called(active: %d, waiting: %d, stalled: %i)\n", sysmillis(), std::this_thread::get_id(), thread_group->active_thread_count.load(), thread_group->waiting_thread_count.load(), thread_group->stalled);
  my_thread_handle thread_id;
...
static int wake_or_create_thread(thread_group_t *thread_group,
  ...
  std::printf("%ld %ld wake or create thread\n", sysmillis(), std::this_thread::get_id());

  if (thread_group->shutdown) DBUG_RETURN(0);

  if (wake_thread(thread_group) == 0) {
    std::printf("%ld %ld thread waked (active: %d, waiting: %d, stalled: %i)\n", sysmillis(), std::this_thread::get_id(), thread_group->active_thread_count.load(), thread_group->waiting_thread_count.load(), thread_group->stalled);
    DBUG_RETURN(0);
  } else {
    std::printf("%ld %ld waked failed (active: %d, waiting: %d, stalled: %i)\n", sysmillis(), std::this_thread::get_id(), thread_group->active_thread_count.load(), thread_group->waiting_thread_count.load(), thread_group->stalled);
  }
  ...
  if (thread_group->active_thread_count == 0 || admin_connection) {
    ...
    */
    std::printf("%ld %ld create worker #1(active: %d, waiting: %d, stalled: %i)\n", sysmillis(), std::this_thread::get_id(), thread_group->active_thread_count.load(), thread_group->waiting_thread_count.load(), thread_group->stalled);
  ...
  if (time_since_last_thread_created >
    ...
    std::printf("%ld %ld throttle create worker #2(active: %d, waiting: %d, stalled: %i)\n", sysmillis(), std::this_thread::get_id(), thread_group->active_thread_count.load(), thread_group->waiting_thread_count.load(), thread_group->stalled);
    DBUG_RETURN(create_worker(thread_group));
    std::printf("%ld %ld after throttle create worker(active: %d, waiting: %d, stalled: %i)\n", sysmillis(), std::this_thread::get_id(), thread_group->active_thread_count.load(), thread_group->waiting_thread_count.load(), thread_group->stalled);
  }
...
static void queue_put(thread_group_t *thread_group, connection_t *connection) {
  ...
  if (thread_group->active_thread_count == 0) {
    std::printf(
        "%ld %ld queue_put > wake_or_create (active: %d, waiting: %d, stalled: %i)\n",
        sysmillis(), std::this_thread::get_id(), thread_group->active_thread_count.load(),
        thread_group->waiting_thread_count.load(), thread_group->stalled);
...
static connection_t *get_event(worker_thread_t *current_thread,
  ...
  std::printf("%ld %ld get_event start (active: %d, waiting: %d, stalled: %i)\n", sysmillis(), std::this_thread::get_id(), thread_group->active_thread_count.load(), thread_group->waiting_thread_count.load(), thread_group->stalled);
  ...
  for (;;) {
    ...
    std::printf("%ld %ld get_event poll (active: %d, waiting: %d, stalled: %i, oversubscribed: %i)\n", sysmillis(), std::this_thread::get_id(), thread_group->active_thread_count.load(), thread_group->waiting_thread_count.load(), thread_group->stalled, oversubscribed);
    if (thread_group->shutdown) break;

    /* Check if queue is not empty */
    if (!oversubscribed) {
      connection = queue_get(thread_group);
      std::printf("%ld %ld get_event connection = %x(active: %d, waiting: %d, stalled: %i, oversubscribed: %i)\n", sysmillis(), std::this_thread::get_id(), connection, thread_group->active_thread_count.load(), thread_group->waiting_thread_count.load(), thread_group->stalled, oversubscribed);
      if (connection) break;
    }

    /* If there is  currently no listener in the group, become one. */
    std::printf("%ld %ld get_event current listener(%x)\n", sysmillis(), std::this_thread::get_id(), thread_group->listener);
    if (!thread_group->listener) {
      thread_group->listener = current_thread;
      thread_group->active_thread_count--;
      std::printf("%ld %ld get_event become listener(active: %d, waiting: %d, stalled: %i)\n", sysmillis(), std::this_thread::get_id(), thread_group->active_thread_count.load(), thread_group->waiting_thread_count.load(), thread_group->stalled);
      mysql_mutex_unlock(&thread_group->mutex);
      std::printf("%ld %ld get_event become listener get lock(active: %d, waiting: %d, stalled: %i)\n", sysmillis(), std::this_thread::get_id(), thread_group->active_thread_count.load(), thread_group->waiting_thread_count.load(), thread_group->stalled);

      connection = listener(thread_group);

      mysql_mutex_lock(&thread_group->mutex);
      std::printf("%ld %ld get_event return from listener(active: %d, waiting: %d, stalled: %i)\n", sysmillis(), std::this_thread::get_id(), thread_group->active_thread_count.load(), thread_group->waiting_thread_count.load(), thread_group->stalled);
      thread_group->active_thread_count++;
        ...
        */
        if (connection_is_high_prio(*connection))
          ...
          */
          ...
          std::printf("%ld %ld get_event too many busy threads(active: %d, waiting: %d, stalled: %i)\n", sysmillis(), std::this_thread::get_id(), thread_group->active_thread_count.load(), thread_group->waiting_thread_count.load(), thread_group->stalled);
          connection->tickets =
        ...
        std::printf("%ld %ld get_event naive connection = %x(active: %d, waiting: %d, stalled: %i, oversubscribed: %i)\n", sysmillis(), std::this_thread::get_id(), connection, thread_group->active_thread_count.load(), thread_group->waiting_thread_count.load(), thread_group->stalled, oversubscribed);
        if (connection) {
          thread_group->queue_event_count++;
          break;
        }
    ...
    */
    std::printf("%ld %ld get_event > wait (active: %d, waiting: %d, stalled: %i)\n", sysmillis(), std::this_thread::get_id(), thread_group->active_thread_count.load(), thread_group->waiting_thread_count.load(), thread_group->stalled);
    ...
    thread_group->active_thread_count++;
    std::printf("%ld %ld get_event > after wakeup(active: %d, waiting: %d, stalled: %i)\n", sysmillis(), std::this_thread::get_id(), thread_group->active_thread_count.load(), thread_group->waiting_thread_count.load(), thread_group->stalled);
  ...
  std::printf("%ld %ld get_event end (active: %d, waiting: %d, stalled: %i)\n", sysmillis(), std::this_thread::get_id(), thread_group->active_thread_count.load(), thread_group->waiting_thread_count.load(), thread_group->stalled);
...
static void wait_begin(thread_group_t *thread_group) noexcept {
  ...
  mysql_mutex_lock(&thread_group->mutex);
  std::printf("%ld %ld wait_begin start (active: %d, waiting: %d, stalled: %i)\n", sysmillis(), std::this_thread::get_id(), thread_group->active_thread_count.load(), thread_group->waiting_thread_count.load(), thread_group->stalled);
  thread_group->active_thread_count--;
  ...
  if ((thread_group->active_thread_count == 0) &&
    ...
    */
    std::printf("%ld %ld wait_begin > wake_or_create(active: %d, waiting: %d, stalled: %i)\n", sysmillis(), std::this_thread::get_id(), thread_group->active_thread_count.load(), thread_group->waiting_thread_count.load(), thread_group->stalled);
    wake_or_create_thread(thread_group);
  }
#endif

  mysql_mutex_unlock(&thread_group->mutex);
  std::printf("%ld %ld wait_begin end (active: %d, waiting: %d, stalled: %i)\n", sysmillis(), std::this_thread::get_id(), thread_group->active_thread_count.load(), thread_group->waiting_thread_count.load(), thread_group->stalled);
  DBUG_VOID_RETURN;
}
...
static void wait_end(thread_group_t *thread_group) noexcept {
  DBUG_ENTER("wait_end");
  std::printf("%ld %ld wait_end start (active: %d, waiting: %d, stalled: %i)\n", sysmillis(), std::this_thread::get_id(), thread_group->active_thread_count.load(), thread_group->waiting_thread_count.load(), thread_group->stalled);
  mysql_mutex_lock(&thread_group->mutex);
  thread_group->active_thread_count++;
  thread_group->waiting_thread_count--;
  mysql_mutex_unlock(&thread_group->mutex);
  std::printf("%ld %ld wait_end end (active: %d, waiting: %d, stalled: %i)\n", sysmillis(), std::this_thread::get_id(), thread_group->active_thread_count.load(), thread_group->waiting_thread_count.load(), thread_group->stalled);
  DBUG_VOID_RETURN;
}
...
bool Thread_pool_connection_handler::add_connection(
  ...
  std::printf("%ld %ld add connection(active: %d, waiting: %d, stalled: %i)\n", sysmillis(), std::this_thread::get_id(), group->active_thread_count.load(), group->waiting_thread_count.load(), group->stalled);
...
static void *worker_main(void *param) {
  ...
  std::printf("%ld %ld worker main start (active: %d, waiting: %d, stalled: %i)\n", sysmillis(), std::this_thread::get_id(), thread_group->active_thread_count.load(), thread_group->waiting_thread_count.load(), thread_group->stalled);
  ...
  my_thread_end();
  std::printf("%ld %ld worker main end(active: %d, waiting: %d, stalled: %i)\n", sysmillis(), std::this_thread::get_id(), thread_group->active_thread_count.load(), thread_group->waiting_thread_count.load(), thread_group->stalled);
  return nullptr;
}
