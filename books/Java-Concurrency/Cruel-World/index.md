...
本章我们来看看这三个特征是什么，以及破坏它们的缘由。

## 内存层次结构

我们先来看看世界的现状。

当前 CPU 算力可以说相当强劲，程序的主要的性能瓶颈其实是 IO，而这集中体现在访问
内存。根据 [Latency Numbers Every Programmer Should
Know](https://colin-scott.github.io/personal_website/research/interactive_latency.html)
里的数据，2020 年，访问 L1 缓存需要 1ns，而访问内存/主存则需要 100ns。因此下面
我们会看到，不管是硬件还是软件上的优化，很多优化都是为了提高 IO 访问的效率。
...
（要注意的是，现代的 CPU 都有极多的优化，我们这里讨论只在乎缓存的理论模型，而
不关心真实实现的细节）
...
哪一种效率更高呢？它们的区别仅仅只在于优先遍历行（左）还是俦遍历列（右）。事实
上左边代码的效率远高于右边的代码，这是因为通常行的数据在内存里是连续排布的，按
行遍历时，该行下一列元素的缓存命中率会比按列遍历时，该列一下个元素的命中率高很
多，而这又会极大地影响程序的效率。
...
JVM 并不保证这些 3 个字节码作为一个整体是原子的，也许因为其它线程的代码，导致
执行时在 #2和 #3 之间插入了另一个 `reg0 = reg0 + 1` 的操作，从而 `++` 执行后，
结果和我们的预期不同。

这里的关键是，即使字节码粒度上每条指令都是原子的（实际并不是），在 Java 操作的
粒度上，也不保证就是原子的；即使 Java 的所有操作都是原子的，没有额外处理的话，
我们写的 Java 函数也不会是原子的。

### 上层的原子能力依赖下层

之前章节中已经谈到 Sequential Consistency 要求每个操作都是原子的，也提到了
Dekker 算法，能够从软件层面实现互斥锁，换句话说可以实现多个操作的原子能力。

当然，Java 已经提供内置锁(Intrinsic Lock/Monitor Lock) 等语义帮助我们方便地实
现多个 Java 操作整体的原子性（可以是一个函数，可以是几行语句），但我们要意识到
Java 在实现这些机制时依赖了操作系统提供的原子能力。
...
2. 原子性如果缺少了可见性，通常就没太大用处，而可见性的实现代价太高了，不适合
   作为默认选项。
...
如果说可见性是一个客观事实，那么有序性就是一个主观行为。程序的重排序一般有两方
面：编译器在编译时会对代码进行重排序；CPU 在执行指令时可能会乱序执行，除此之外
Java 使用的 JIT 也对执行的指令有重排序。而这样做的目的，都是为了提高性能。下面
举几个重排序的例子[^reordering-example]：
...
一定要在另一个线程的 `read`操作前执行，因此无法合理地做出推断。那要怎么办呢？
