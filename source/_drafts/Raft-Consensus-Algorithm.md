...
{% asset_svg raft-replicated-state-machine.svg Replicated State Machine %}
...
{% asset_svg raft-server-states.svg Server States %}

其次是任期（term）的概念。Raft 将时间切分成多个 term，每个 term 以选主开始，选
主期间各节点尝试当上主节点，选举结束后开始正常处理客户端的请求。如图：

{% asset_svg raft-term-definition.svg Term Definition %}
...
{% asset_svg raft-log-entries.svg Log Entries %}
...
{% asset_svg raft-log-inconsistency.svg Log Inconsitency %}
...
{% asset_svg raft-log-safety.svg Log Safety Example %}
...
{% asset_svg raft-algorithm.svg 100% Raft Algorithm %}
...
{% asset_svg raft-disjoint-majorities.svg Two disjoint Majorities Problem %}
...
{% asset_svg raft-add-delete-single-node.svg Add Or Remove A Single Node %}
...
{% asset_svg raft-membership-algorithm.svg full Two disjoint Majorities Problem %}
