...
即在执行 `vector.push_back` 时，`elem` 指向的内存已经被释放了，造成了“访问已
释放内存”的问题。也许程序不会直接崩溃，但极可能得到的错误的结果。

上面的例子中，产生“指针安全”的原因主要有两个：
...
Rust 提出的方案是：Ownership（所有权）及 Borrowing（租借），即：
...
*2*. 允许租借。你可以先把书“给”别人，别人用完后再“给”你。“借”，则保证对方不会
不把书还你。例如：
...
如果有一个人将书借去“写”，则不允许其它人同时“读”，即 rust 只允许有一个不可变的
引用(&mut T)：
...
一个很直观的想法（不代表实际实现）是：为每个变量维护一个集合，集合里记录该变量
的引用（Reference，也就是租借），那么编译器在分析时就确保 #1, #2.1, #2.2。
为了确保规则 #3，rust 编译器需要确保一个资源的 reference 的存在时间不会比资源
的 Owner 还长。
...
上面的例子较为简单，编译器也可以做一些自动的分析，但还有一些情况下，编译器并没
有办法知道生命周期的限制，例如：
...
可以看到，如果 `foo` 返回 `x` 的值，则上例的 `main` 函数是合法的，但当 `foo`
返回 `y` 时，①处 `y` 作用域结束，但 `z` 依旧持有 `y` 的引用，违反规则 #3。

所以这里的问题是单凭静态分析本身并没有办法确定所有的生命周期，因此需要一定的人
工介入，给编译器一些提示：
...
上述标识告诉编译器，函数 `foo` 的返回值的生命周期，要小于任意参数的生命周期。
...
z:             |_______________|
y:                  |__________|
违反函数 foo 的要求： Lifetime(z) <= Lifetime(x) ∩ Lifetime(y)
```
...
上述代码 rust 认为①处存在对变量 `a` 的引用，原因是 `c` 是对 `a` 的引用，并且在
...
回值的 Lifetime `'a`，它小于变量 `x` 的生命周期，因此上述代码能通过静态分析，
但若运行时函数 `foo` 返回了 `y`，则又产生了内存安全的问题。

因此，即使 rust 需要我们人工指定一些生命周期，它对指定的内容也是有要求的，要求
...
让我们付出的代价是性能，而 rust 需要的则是程序员更多的学习时间。
