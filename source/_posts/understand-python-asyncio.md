...
与线程不同的是，协程完成的功能通常较小，所以会有需求将不同的协程串起来，我们
...
里就可以了。第二个问题，可以监听什么事件？由于 `poll_events` 需要操作系统的
...
`register_event` 用于注册 `事件: 回调` 的关系，`start_loop` 用于开启事件循环。

现在，你可能想说，之前提到过事件也包括“某个计时器归零了”，但 `poll_events` 只
支持网络 I/O 的文件描述符，计时器又要如何实现呢？一般 `poll_events` 函数支持
`timeout` 参数表示等待的时间。因此可以修改 `start_loop`:
...
这里我不禁想问，为什么大家没有一开始就想到 `async/await` 的方式呢？我的一个猜
想是 `async/await` 是需要语言本身的支持的，而写编译器/解释器的专家不一定有编写
